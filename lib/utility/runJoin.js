...
exports.runJoin = function runJoin(Model, isCollection, instances, models, isInnerJoin, next) {
	...
	function queryModel(model, next) {
		...
		self.logger.debug('Querying ' + model.name + ' where ' + JSON.stringify(query));
		self.execModelMethod(Model, model, 'query', {
			where: query,
			limit: calculateLimit(model)
		}, function queryCallback(err, result) {
			if (err) { return next(err); }
			self.logger.debug('Performing', isInnerJoin ? 'inner join' : 'left join');
	...
	function calculateLimit(model) {
		...
		self.logger.debug('Set query limit to', limit);
		return limit;
	}
	...
	function returnInstance(err) {
		...
		self.logger.debug('Populating model,', Model.name, 'with joined data:\n', JSON.stringify(retVal, null, 2));
		var instance = Model.instance(retVal, true);
		instance.setPrimaryKey(retVal.id);
		self.logger.debug('Returning composed model (' + Model.name + ') instance:\n', JSON.stringify(instance, null, 2));
		next(null, instance);
	}
};
