...
exports.execComposite = function execComposite(params) {
	...
	async.each(joinedModels, function modelExecHandler(model, cb) {
		...
		// On update convert instances to their source model instance and map aliases.
		if (params.isWrite && localArg.toPayload && localArg.primaryKey) {
			var SourceModel = Arrow.getModel(model.name);
			var inst = SourceModel.instance({});
			inst.id = localArg.primaryKey;

			// Re-map the values
			var values = localArg.values(true);
			for (var key in values) {
				if (values.hasOwnProperty(key)) {
					if (params.Model.fields[key].name && key !== params.Model.fields[key].name) {
						values[params.Model.fields[key].name] = values[key];
						delete values[key];
					}
				}
			}
