...
exports.create = function (Arrow, server) {
	...
	return Connector.extend({
		...
		findAll: function findAll(Model, next) {
			execComposite({method: 'findAll', isWrite: false, isCollection: true, Model: Model, arg: next});
		},
	...
	function execComposite(params) {
		...
		for (var fieldName in params.Model.fields) {
			if (params.Model.fields.hasOwnProperty(fieldName)) {
				...
				if (modelMeta) {
					...
					if (params.method === 'query' && containsKey(arg, fieldName, ['sel', 'unsel'])) {
		...
		async.each(joinedModels, function modelExecHandler(model, cb) {
			...
			var fieldKey = fetchModelObjectFieldKey(params.Model, model);
			var localArg = arg;
			...
			if (params.isWrite && !localArg.getPrimaryKey) {
				localArg = _.pick(localArg, _.keys(server.getModel(model.name).fields));
			}
			execModelMethod(params.Model, model, params.method, localArg, function methodCallback(err, instance) {
				if (err) {
					cb(err);
				}
				else {
			...
			if (err) {
				...
				runJoin(params.Model, params.isCollection, instances, joinedModels, isInnerJoin, next);
			}
		});
