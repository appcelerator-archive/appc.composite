...
exports.create = function(APIBuilder, server) {
	...
	function execComposite(method, isWrite, isCollection, Model, arg, next) {
		...
		var left_join = Model.getMeta('left_join'),
			modelMap = {},
			models = [],
			instances = {};
		...
		for (var fieldName in Model.fields) {
			if (Model.fields.hasOwnProperty(fieldName)) {
				...
				var modelName = field.cmodel,
					collectionName = field.ccollection;
				if (modelName) {
					if (modelMap[modelName]) {
						continue;
					}
					if (left_join.model === modelName) {
						models.push({
							name: modelName,
							readonly: left_join.readonly,
							left_join: left_join.join_properties,
							multiple: left_join.multiple
						});
					}
					else {
						models.unshift({
